...
# LoRA keys saved by HF text SFT vs vLLM multimodal Qwen3.5 language tower.
_LORA_OLD_KEY = "base_model.model.model.layers."
_LORA_NEW_KEY = "base_model.model.model.language_model.layers."


def is_qwen35_model(model_path: str | Path) -> bool:
    cfg_path = Path(model_path) / "config.json"
    if not cfg_path.exists():
        return False
    cfg = json.loads(cfg_path.read_text())
    archs = cfg.get("architectures") or []
    return any("Qwen3_5" in str(a) for a in archs)


def vllm_text_only_kwargs() -> dict:
    """Disable vision inputs for text-only chart tasks on Qwen3.5."""
    return {"limit_mm_per_prompt": {"image": 0}}


def checkpoint_kind(path: Path | str | None) -> str | None:
    """Return ``lora``, ``full``, or None if *path* is not a usable checkpoint."""
    ...
    if (p / "config.json").exists() and index.exists():
        return "full"
    return None


def latest_usable_checkpoint(path: Path | str | None) -> Path | None:
    if path is None:
        return None
    p = Path(path)
    if not p.exists():
        return p
    if checkpoint_kind(p) is not None:
        return p
    ckpts = sorted(
        p.glob("checkpoint-*"),
        key=lambda item: int(item.name.split("-")[-1])
        if item.name.split("-")[-1].isdigit() else -1,
    )
    for ckpt in reversed(ckpts):
        if checkpoint_kind(ckpt) is not None:
            return ckpt
    return p


def ensure_renamed_lora_adapter(adapter_dir: str | Path) -> str:
    """Rename LoRA keys for vLLM Qwen3.5 language tower (idempotent)."""
    import os
    import shutil
    ...
    if not any(k.startswith(_LORA_OLD_KEY) for k in sd):
        return str(src)
    ...
    n = 0
    for k, v in sd.items():
        if k.startswith(_LORA_OLD_KEY):
            new_sd[k.replace(_LORA_OLD_KEY, _LORA_NEW_KEY, 1)] = v
            n += 1
        else:
            new_sd[k] = v
    save_file(new_sd, str(dst / "adapter_model.safetensors"))
    print(f"  [rename] {n}/{len(sd)} LoRA keys → {dst}")
    return str(dst)


def sync_vllm_multimodal_assets(base_model: str | Path, checkpoint_dir: str | Path) -> None:
    """Copy vision preprocessor files required by vLLM Qwen3.5 loading."""
...
def freeze_qwen35_vision(model) -> None:
    for name, param in model.named_parameters():
        if any(tag in name for tag in ("visual", "vision", "vision_tower")):
