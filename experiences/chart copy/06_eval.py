...
def evaluate_model(base_model_path, checkpoint_path, questions, max_new_tokens=512, batch_size=4):
    model_path, lora_path = resolve_sft_checkpoint(base_model_path, checkpoint_path)
    tok_path = checkpoint_path or base_model_path
    tokenizer = AutoTokenizer.from_pretrained(tok_path, trust_remote_code=True, padding_side="left")
    ...
    name = Path(checkpoint_path).name if checkpoint_path else "base"
    correct = 0
    results = []
...
def main():
    ...
    for ds_name in args.datasets:
        ...
        for cond in args.conditions:
            if cond == "base":
                continue
            ckpt = adapter_map.get(cond)
            if ckpt and Path(ckpt).exists():
                results[f"{cond}_sft"] = evaluate_model(
                    model_path, ckpt, questions, max_new_tokens, batch_size,
                )
            else:
                print(f"  skip {cond}: checkpoint missing at {ckpt}")
