...
from utils import chart_answers_match_4dp, extract_chart_answer
...
def main():
    ...
    with open(para_in) as f:
        for line in f:
            raw_loaded += 1
            r = json.loads(line)
            extracted = r.get("paraphrase_extracted")
            if extracted is None:
                extracted = extract_chart_answer(r["paraphrase_text"])
            if not chart_answers_match_4dp(extracted, r["answer"]):
