# WARNING: 05_sft_train.py task-branch auto-rewrite failed; please manually replace the task-dispatch in main() with hard-coded CHART block.
#!/usr/bin/env python3
"""Step 5: SFT on paraphrase (or raw) training data — LoRA or full fine-tuning."""
...
def _prepare_model(model, sft_cfg: dict):
...
def _training_precision(sft_cfg: dict) -> tuple[bool, bool]:
    if _use_lora(sft_cfg):
        return False, False
    return True, False
...
def train_one_vision(model_path, train_data_path, output_dir, sft_cfg, system_prompt,
    ...
    model = _prepare_model(model, sft_cfg)

    batch_size = sft_cfg.get("batch_size", 2)
    grad_accum = sft_cfg.get("grad_accum", 4)
    bf16, fp16 = _training_precision(sft_cfg)
...
def main():
    ...
    p.add_argument("--condition",
                   ...
                   help="'both' = raw + paraphrase (token-matched pair); "
                        ...
                        "'para_clear' = meta-text-cleaned paraphrase.")
    p.add_argument("--config", type=str, default=None)
    p.add_argument("--tuning", choices=["lora", "full"], default=None,
                   help="Override sft.tuning in config (default: read from config).")
