...
def checkpoint_kind(path: Path | str) -> str | None:
...
def latest_usable_checkpoint(path: Path | str | None) -> Path | None:
    """Return a LoRA/full checkpoint dir, preferring the latest ``checkpoint-*`` subdir."""
    if path is None:
        return None
    p = Path(path)
    if not p.exists():
...
def resolve_sft_checkpoint(base_model_path: str | Path,
                           checkpoint_dir: str | Path | None) -> tuple[str, str | None]:
    """Return ``(model_path, lora_adapter_path)`` for HF / vLLM loading.

    * ``checkpoint_dir is None`` → base model only.
    * LoRA dir → ``(base_model_path, checkpoint_dir)``.
    * Full SFT dir → ``(checkpoint_dir, None)``.
    """
    if not checkpoint_dir:
        return str(base_model_path), None
    kind = checkpoint_kind(checkpoint_dir)
    ckpt = str(checkpoint_dir)
    if kind == "lora":
        return str(base_model_path), ckpt
    if kind == "full":
        return ckpt, None
    return str(base_model_path), None
...
def _to_decimal_4dp(s: str) -> Decimal | None:
    try:
...
def _normalize_chart_answer_4dp(ans: Any) -> str | Decimal:
    s = normalize_chart_answer(ans)
    if not s:
        return ""
    dec = _to_decimal_4dp(s)
    if dec is not None:
        return dec
    return s.replace(" ", "")
...
def chart_answers_match_4dp(pred: str | None, gold: Any) -> bool:
    """Strict SFT matching: numbers compare after 4dp rounding, text exact."""
    if pred is None:
        return False
    p = _normalize_chart_answer_4dp(pred)
    g = _normalize_chart_answer_4dp(gold)
    if isinstance(p, Decimal) and isinstance(g, Decimal):
        return p == g
