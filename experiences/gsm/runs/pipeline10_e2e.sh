#!/usr/bin/env bash
# GSM8K full pipeline smoke: 10 train questions, G=4, data4 FTSO only.
set -euo pipefail
...
N=10
LOG="$LZL_ROOT/logs/gsm/pipeline10/e2e_$(date +%Y%m%d_%H%M%S).log"
...
mkdir -p "$(dirname "$LOG")" /data4/FTSO/datasets/gsm/pipeline10
exec > >(tee -a "$LOG") 2>&1

echo "============================================================"
echo "=== GSM pipeline10 e2e  $(date)"
...
echo; echo "[6/6] eval (gsm8k_test limit=$N, base + 3 adapters)"
...
EVAL_JSON="$RESULTS/eval/gsm8k_test.json"
$PY - "$EVAL_JSON" <<'PY'
import json, sys
...
echo; echo "============================================================"
echo "=== GSM pipeline10 e2e DONE  $(date)"
echo "=== LOG: $LOG"
echo "============================================================"
