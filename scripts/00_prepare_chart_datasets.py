#!/usr/bin/env python3
"""Download/prepare chart datasets → /data5/lzl raw + VCTS/lzl/data/chart JSONL."""
...
from chart_utils import format_chartqa, format_finqa, format_tabmwp
from paths import ensure_dirs, load_config

RAW_ROOT = Path("/data5/lzl/datasets")
CHART_ROOT = Path(__file__).resolve().parent.parent / "data" / "chart"
PLOTQA_EVAL_N = 5000
PLOTQA_EVAL_PATH = CHART_ROOT / "eval" / "plotqa_eval_5k.jsonl"
CHARTQA_ZIP_CANDIDATES = [
    RAW_ROOT / "chartqa" / "raw" / "ChartQA Dataset.zip",
...
def _ensure_chartqa_extracted() -> Path:
...
def export_chartqa_test(max_eval: int | None = None):
    """Export the official ChartQA *test* split (human + augmented merged).

    The training counterpart is built by `export_chartqa()`; this fn mirrors it
    for evaluation, producing data/chart/eval/chartqa_test_eval.jsonl with the
    same schema as the other chart eval JSONLs (PlotQA / TabMWP / FinQA).
    """
    print("\n[ChartQA test] building eval JSONL from official zip …")
    root = _ensure_chartqa_extracted()
    test_dir = root / "test"
    rows = []
...
def export_tabmwp(max_eval: int | None):
...
def export_finqa(max_eval: int | None):
...
def export_plotqa(n_eval: int = PLOTQA_EVAL_N, seed: int = 42):
    ...
    save_jsonl(rows, PLOTQA_EVAL_PATH)
...
def main():
    ...
    if _do("chartqa"):       export_chartqa()
    if _do("chartqa_test"):  export_chartqa_test(max_eval)
    if _do("tabmwp"):        export_tabmwp(max_eval)
    if _do("finqa"):         export_finqa(max_eval)
